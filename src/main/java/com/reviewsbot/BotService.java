...
import org.telegram.telegrambots.meta.api.objects.PreCheckoutQuery;
import org.telegram.telegrambots.meta.api.objects.SuccessfulPayment;
...
public class BotService extends TelegramLongPollingBot {
    ...
    private void handleMessage(Message message) throws Exception {
        ...
        if ("/add".equalsIgnoreCase(text)) {
            if (user.isAdmin()) {
                ...
                sendText(message.getChatId(), "Отправьте фото мужчины или напишите 'Пропустить'.");
            } else {
                sendText(message.getChatId(), "Доступ запрещён.");
            }
            return;
    ...
    private void handleStart(User user, Message message) throws Exception {
        if (user.isAdmin()) {
            ...
            sendText(message.getChatId(), "Добро пожаловать, администратор.");
            sendMainMenu(message.getChatId(), user);
            return;
        }
        if (user.state() == UserState.WAIT_SECRET) {
    ...
    private void handleSecret(User user, String code, long chatId) throws Exception {
        if (config.secretCode.equals(code)) {
            db.updateUserState(user.tgId(), UserState.NONE, null);
            sendText(chatId, "Доступ подтверждён. Добро пожаловать!");
    ...
    private void handleFindCallback(CallbackQuery cb, User user) throws Exception {
        ...
        switch (cb.getData()) {
            case "find:phone" -> {
                ...
                sendText(chatId, "Отправьте номер телефона текстом.");
            }
            case "find:tg" -> {
                db.updateUserState(user.tgId(), UserState.WAIT_TG_FOR_FIND, null);
                sendText(chatId, "Отправьте Telegram username (например @user) или ID.");
            }
            case "find:name" -> {
    ...
    private void handleFindByPhone(User user, long chatId, String phone, String flow) throws Exception {
        ...
        if (man == null) {
            ...
            if ("review".equals(flow)) {
                ...
                sendText(chatId, "Запись не найдена. Создаём карточку. Отправьте фото или 'Пропустить'.");
            } else {
    ...
    private void handleManPhoto(User user, Message message) throws Exception {
        ...
        if (message.hasPhoto()) {
            ...
            sendText(message.getChatId(), "Отправьте фото или напишите 'Пропустить'.");
        }
    }
    ...
    private void handleManName(User user, Message message) throws Exception {
        ...
        sendText(message.getChatId(), "Добавьте описание или напишите 'Пропустить'.");
    }
    ...
    private void handleReviewText(User user, Message message) throws Exception {
        ...
        sendText(message.getChatId(), "Отзыв отправлен на модерацию.");
        if (review != null) {
            notifyAdminsForReview(review.id());
        }
        Man man = db.getManById(manId);
        if (man != null) {
            sendManCard(message.getChatId(), user, man, true);
        }
    }
    ...
    private void handleEditReviewText(User user, Message message) throws Exception {
        ...
        db.clearUserState(user.tgId());
        sendText(message.getChatId(), "Отзыв обновлён и отправлен на модерацию.");
    ...
    private void handleAdminAddDesc(User user, Message message) throws Exception {
        ...
        sendText(message.getChatId(), "Введите телефон или @username (можно пропустить).");
    }
    ...
    private void handleAdminSetPrices(User user, Message message) throws Exception {
        ...
        db.clearUserState(user.tgId());
        sendText(message.getChatId(), "Цены обновлены.");
    }
    ...
    private void startCreateMan(User user, long chatId) throws Exception {
        ...
        sendText(chatId, "Отправьте фото мужчины или напишите 'Пропустить'.");
    }
    ...
    private void handleRateCallback(CallbackQuery cb, User user) throws Exception {
        ...
        sendText(cb.getMessage().getChatId(), "Отправьте комментарий или напишите 'Пропустить'.");
        answer(cb);
    }
    ...
    private void handleEditRateCallback(CallbackQuery cb, User user) throws Exception {
        ...
        sendText(cb.getMessage().getChatId(), "Отправьте новый текст или напишите 'Пропустить'.");
        answer(cb);
    }
    ...
    private void handleAdminCallback(CallbackQuery cb, User user) throws Exception {
        ...
        switch (action) {
            ...
            case "prices" -> {
                ...
                sendText(chatId, "Текущие цены: week=" + w + " month=" + m + " single=" + s + " single_days=" + d +
                        "\nОтправьте новые значения в формате week=200 month=500 single=99 single_days=0");
                db.updateUserState(user.tgId(), UserState.WAIT_ADMIN_SET_PRICES, null);
            }
            case "addman" -> {
    ...
    private void sendFindMethod(long chatId, String flow) throws Exception {
        List<List<InlineKeyboardButton>> rows = new ArrayList<>();
        if ("find".equals(flow)) {
            rows.add(List.of(btn("📱 Отправить номер телефона", "find:phone")));
            rows.add(List.of(btn("✍️ Ввести Telegram ID / Username", "find:tg")));
            rows.add(List.of(btn("🔎 Поиск по имени", "find:name")));
        } else {
            rows.add(List.of(btn("📱 Отправить номер телефона", "review:phone")));
            rows.add(List.of(btn("✍️ Ввести Telegram ID / Username", "review:tg")));
        }
        SendMessage sm = new SendMessage(String.valueOf(chatId), "Выберите способ ввода:");
        sm.setReplyMarkup(new InlineKeyboardMarkup(rows));
        execute(sm);
    }
    ...
    private void sendPaywall(long chatId, Man man) throws Exception {
        ...
        String text = "У вас нет доступа к просмотру отзывов.\n" +
    ...
    private void approveReview(int reviewId, User admin) throws Exception {
        ...
        if (author != null) {
            sendText(author.tgId(), "Ваш отзыв одобрен.");
        }
    ...
    private void sendRatingButtons(long chatId, int id, boolean isEdit) throws Exception {
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            row.add(btn("⭐".repeat(i), data));
        }
        rows.add(row);
        SendMessage sm = new SendMessage(String.valueOf(chatId), "Выберите оценку:");
        sm.setReplyMarkup(new InlineKeyboardMarkup(rows));
        execute(sm);
    }
    ...
    private String buildProviderData(String title, int amountRub) {
        String value = String.format(Locale.US, "%.2f", amountRub);
